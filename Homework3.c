        ...
        if (i == 1) {
            ...
            switch(key) {
                ...
                case 1094: case 1062: // Ц ц in Russian layout
                   //если до этого змея не двигалась вниз-меняем направление на ВВЕРХ
                ...
                case 1099: case 1067: // Ы ы in Russian layout
                   //если до этого змея не двигалась вверх-меняем направление на ВНИЗ
                ...
                case 1092: case 1060: // Ф ф in Russian layout
                   //если до этого змея не двигалась вправо-меняем направление на ЛЕВО
                ...
                case 1074: case 1042: // В в in Russian layout
                     //если до этого змея не двигалась влево-меняем направление на ВПРАВО
            ...
            //такие же проверки при изменении направления движения пр иуправлении с помощью WASD
            if ((key == snakes[i].controls.down)&&(snakes[i].direction!=UP))
                snakes[i].direction = DOWN;
            else if ((key == snakes[i].controls.up)&&(snakes[i].direction!=DOWN))               
                   snakes[i].direction = UP;
            else if ((key == snakes[i].controls.right)&&(snakes[i].direction!=LEFT))
                    snakes[i].direction = RIGHT;
            else if ((key == snakes[i].controls.left)&&(snakes[i].direction!=RIGHT))            
                    snakes[i].direction = LEFT;
        }
    }
}
...
int isCrush(snake_t *snake) {
    ...
    //проверяем для головы и хвоста: если  координаты х и у совпадают-змея врезалась  сама в себя
    //возвращаем 1
...
int checkAllCollisions(snake_t snakes[], size_t num_snakes, int max_x, int max_y) {
    ...
    for (size_t i = 0; i < num_snakes; i++) {
        ...
        // Check self-intersection проверяем врезалась ли змея сама в себя
        if (isCrush(&snakes[i])) {
