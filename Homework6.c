...
//считывание символа без задержек
int getch_without_delay(int milliseconds) {
    if (milliseconds <= 0) {
...
void showWelcomeScreen() {
    ...
    mvprintw(30, 8, "Arrow keys UP DOWN - Navigate options");
...
void playGame() {
    ...
    while (key_pressed != STOP_GAME && !game_over && game_running) {
        ...
        key_pressed = getch_without_delay(100);
