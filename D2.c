...
//функция для поиска узла дерева с заданным ключем
tree* find_Key(tree* root, int key) 
{
    if (root != NULL) 
    {
    
		//если ключ совпал с ключем текущего узла
		if (root->key == key) 
		{return root;}
		else
		{
			//иначе-ищем в левом поддереве
			tree* left = find_Key(root->left, key);
				if (left!=NULL) 
				{return left;}
				else
				{
					//если не нашли в левом поддереве-ищем в правом поддереве
...
//функция, которая по переданному ключу возвращает адрес соседнего элемента - брата
tree * findBrother(tree *root, int key)
{
	tree* result; 
	tree* parent;
	//ищем,существует ли узел с таким ключем
	result= find_Key(root, key);
	...
	{
		//если такой узел  существует
		if (result->parent == NULL) 
		{return NULL;}
		
		//нужно найти "брата" узла-сим.распол. отн. род.узла
		 parent = result->parent;
		//если узел  и скомым ключем  в левом поддереве род.узла
		//то его брат-в правом
		if (parent->left == result) 
		{
			return parent->right;
		}
		//если узел  с иск.ключем в правом поддереве-его брат в левом поддереве
...
int main(int argc, char **argv)
{
	...
	brother=findBrother(tr, key);
	
	printf("%d",brother->key);
